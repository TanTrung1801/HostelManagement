...
--changeset lekha:1653760916797-43
DROP TRIGGER IF EXISTS `insert_trigger_date_status_contract`;
CREATE DEFINER=`root`@`localhost` TRIGGER `insert_trigger_date_status_contract` AFTER INSERT 
ON `contracts` 
FOR EACH ROW 
    UPDATE contract 
...
--changeset lekha:1653760916797-44
DROP TRIGGER IF EXISTS `update_trigger_date_status_contract`;
CREATE DEFINER=`root`@`localhost` TRIGGER `update_trigger_date_status_contract` AFTER UPDATE 
ON `contracts` 
FOR EACH ROW 
    UPDATE contract
    SET contract.contract_status = 0 
...
--changeset lekha:1653760916797-45
